    ...
    <div className="product-card">
      <div className="product-image">
        ...
        <div className="wishlist-and-cart-box">
          {loginData.token && isItemInBasket(_id) ? (
            <IoCart className="cart-icon-fill" />
          ) : (
            <IoCartOutline
              title="Cart"
              className="cart-icon"
              onClick={(e) => addToBasketHandler(product, loginData.token)}
            />
          )}
          <IoIosSearch className="quick-view" />
          {loginData.token && isItemInWishlist(_id) ? (
            <IoMdHeart
